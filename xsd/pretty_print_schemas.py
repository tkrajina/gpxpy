...
def parse_1_1_element_node(gpx_element_node, complex_type_nodes, depth=0):
    ...
    if node_type in complex_type_nodes:
        ...
        for attribute_node in attribute_nodes:
            ...
            result += get_indented(depth + 2, 'attr: %s (%s) %s' % (attribute_node_name, attribute_node_type, attribute_node_required))
...
def parse_1_0_elements(element, depth=0):
    ...
    complex_types = element.getElementsByTagName('xsd:complexType')
    if len(complex_types) > 0:
        for sub_element in complex_types[0].getElementsByTagName('xsd:sequence')[0].getElementsByTagName('xsd:element'):
            result += parse_1_0_elements(sub_element, depth=depth+1)
    for attribute_node in element.getElementsByTagName('xsd:attribute'):
        attribute_node_name = attribute_node.attributes['name'].value
        attribute_node_type = attribute_node.attributes['type'].value
        attribute_node_use = attribute_node.attributes['use'].value if attribute_node.attributes.has_key('use') else None
        result += get_indented(depth + 2, 'attr: %s (%s) %s' % (attribute_node_name, attribute_node_type, attribute_node_use))
    return result
