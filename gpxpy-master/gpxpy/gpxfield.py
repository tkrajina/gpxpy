...
class GPXExtensionsField(AbstractGPXField):
    ...
    def from_xml(self, parser, node, version):
        ...
        extensions_node = parser.get_first_child(extensions_node, self.tag2)

        if extensions_node is None:
            return result
        
        # Needs to be child of a child - in v1.1, extensions are hidden behind a <gpxtpx:TrackPointExtension> tag
